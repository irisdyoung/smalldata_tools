...
cat << EOF
$(basename "$0"): 
	Script to monitor presence of FEE, ebeam, and diffraction detector in each event
...
#Generate python args, for now if
...
LOGFILE='monitor_evts_'${EXPERIMENT}'_run'${RUN_NUM}'_%J.log'
if [ -v LOGDIR ]; then
    if [ ! -d "$LOGDIR" ]; then
        mkdir -p "$LOGDIR"
    fi
...
SERIAL_CMD="python ${ABS_PATH}/${PYTHONEXE} $*"
# not yet multiprocessing enabled
...
if [[ $QUEUE == *milano* ]]; then
    ...
    if [[ $ACCOUNT == 'lcls' ]]; then
        echo $SBATCH_ARGS --qos preemtable --account $ACCOUNT --wrap="$SERIAL_CMD"
	    sbatch $SBATCH_ARGS --qos preemptable --account $ACCOUNT --wrap="$SERIAL_CMD"
    else
        echo ---- $ABS_PATH/$PYTHONEXE $@
        echo $SBATCH_ARGS --account $ACCOUNT --wrap="$SERIAL_CMD"
	    sbatch $SBATCH_ARGS --account $ACCOUNT --wrap="$SERIAL_CMD"

    fi
else # for outside s3df
    sbatch $SBATCH_ARGS --wrap "$SERIAL_CMD"
fi
